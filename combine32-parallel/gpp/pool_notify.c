 ...
 *  ============================================================================
 */
    Uint32 * pool_notify_DataBuf = NULL ;
 ...
 */
    ...
    // Initialize the array with something
       ...
       pool_notify_DataBuf[i] = 0;//  i % 20 + i % 5;
   }
}
 ...
 *  ============================================================================
 */
NORMAL_API DSP_STATUS pool_notify_Execute (IN Uint32 numIterations, 
    Uint8 processorId, 
    int32_t SIZE,
    int32_t *matrixA,
    int32_t *matrixB,
    int32_t *matrixResult, 
    long long *time)
{
    DSP_STATUS  status    = DSP_SOK ;
    ...
    long long start1;
    int i, j;
    int startBufferAddress = 2*SIZE*SIZE;    
    ...
    storeMatrixToBuffer(SIZE,matrixA,matrixB,pool_notify_DataBuf);
    // #ifdef DEBUG
    // printPoolBuffer(SIZE, pool_notify_DataBuf);
    // #endif 
    ...
    matrixMultiplicationNEONThreePart(SIZE, matrixA, matrixB, matrixResult);
	...
	#endif
    *time = get_usec()-start1;

// copy results from buffer to matrix result   
    for (i = 0;i < SIZE/4; i++)
    {   
        for (j = 0; j < SIZE; j++)
        {
            matrixResult[i*SIZE+j]=(int32_t)pool_notify_DataBuf[startBufferAddress + i*SIZE + j];
        }
    }
    
    // start2 = get_usec();
    // // matrixMultiplicationNEON(SIZE, matrixA, matrixB, matrixResult);
    // *time = *time + (get_usec() - start2);

    // #ifdef DEBUG
    // printResultsBuffer(SIZE, pool_notify_DataBuf);
    // #endif 
    ...
    long long *time)
{
    ...
    if (   (dspExecutable != NULL)   ) 
    {
		...
		// convert matrix size to buffer size (in 2 bytes/buffer)
		bufferSize = 4*3*SIZE*SIZE; // 2 input and 1 output
		
        pool_notify_BufferSize = DSPLINK_ALIGN (bufferSize,
          DSPLINK_BUF_ALIGN) ;		
        ...
        if (DSP_SUCCEEDED (status)) 
        {   
         status = pool_notify_Execute (pool_notify_NumIterations, 0, SIZE, matrixA, matrixB, matrixResult, time) ;
     }
...
STATIC Void pool_notify_Notify (Uint32 eventNo, Pvoid arg, Pvoid info)
{
    ...
    {
         isFinished = 1;
        printf(" Time to do matrix multiplication on DSP is %d \n", (int)info);
    }
}	

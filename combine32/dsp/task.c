...
Int Task_create (Task_TransferInfo ** infoPtr)
{
  ...
  void matMult()
  {
   ...
   int index_start_out = 2*SIZE*SIZE;
   int index_start_two = SIZE*SIZE;
   ...
   for (i = 0;i < SIZE; i++) 
   {
    ...
    {  
      a = SIZE*i;
      index = index_start_out + a + j;
      buf[index] = 0;
      for(k=0;k<SIZE;k++){
        b =k*SIZE;
        buf[index] = buf[index] +  buf[a + k] * buf[index_start_two + b + j];
      }
    }
  }
}	
...
Int Task_execute (Task_TransferInfo * info)
{
  ...
  SEM_pend (&(info->notifySemObj), SYS_FOREVER);
	...
	//invalidate cache
  BCACHE_inv ((Ptr)buf, 2*4*SIZE*SIZE, TRUE) ;

  // perform matrix multiplication
  matMult();
  
  // write from DSP cache to shared memory
  BCACHE_wb((Ptr)&buf[2*SIZE*SIZE], 4*SIZE*SIZE, TRUE);	

	//notify that we are done
  NOTIFY_notify(ID_GPP,MPCSXFER_IPS_ID,MPCSXFER_IPS_EVENTNO,(Uint32)0);

	// notify the amount of time required
  // NOTIFY_notify(ID_GPP,MPCSXFER_IPS_ID,MPCSXFER_IPS_EVENTNO,(Uint32)total);
  NOTIFY_notify(ID_GPP,MPCSXFER_IPS_ID,MPCSXFER_IPS_EVENTNO,(Uint32)SIZE);
